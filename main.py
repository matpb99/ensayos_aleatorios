...
def get_random_elements(root, n, seed=None):
    ...
    for category_folder in categories_folders:
        ...
        random_elements_per_category.append(random.sample(category_elements_images, num_samples))
...
def _get_layout_pages_for_category(category_images, placement_strategy_func):
    ...
    for img_path, img_height in images_with_heights:
        placement_strategy_func(pages, page_heights, img_path, img_height, current_max_page_height)
...
def _first_fit_placement(pages, page_heights, img_path, img_height, max_page_height):
    ...
    for i, page in enumerate(pages):
        if len(page) < ITEMS_PER_PAGE and page_heights[i] + img_height + ITEM_MARGIN <= max_page_height:
...
def _best_fit_placement(pages, page_heights, img_path, img_height, max_page_height):
    ...
    for i, page in enumerate(pages):
        if len(page) < ITEMS_PER_PAGE and page_heights[i] + img_height + ITEM_MARGIN <= max_page_height:
            remaining_space = max_page_height - (page_heights[i] + img_height + ITEM_MARGIN)
...
def get_final_layout(categories_list, seed):
    ...
    for category_images in categories_list:
        all_pages.extend(_get_layout_pages_for_category(category_images, _first_fit_placement))
    return all_pages
...
def get_final_layout2(categories_list, seed):
    ...
    all_pages = []
    for category_images in categories_list:
        all_pages.extend(_get_layout_pages_for_category(category_images, _best_fit_placement))
    return all_pages

def add_header(canvas_pdf, title, seed):
    canvas_pdf.setFont("Helvetica-Bold", 16)
    canvas_pdf.drawCentredString(MAX_PAGE_WIDTH / 2, MAX_PAGE_HEIGHT - 15 - 5, title.capitalize())

    canvas_pdf.setLineWidth(1)
    canvas_pdf.line(PAGE_MARGIN, MAX_PAGE_HEIGHT - 15 - 10, MAX_PAGE_WIDTH - PAGE_MARGIN, MAX_PAGE_HEIGHT - 15 - 10) 

    canvas_pdf.setFont("Helvetica", 12)
    canvas_pdf.drawRightString(MAX_PAGE_WIDTH - PAGE_MARGIN, MAX_PAGE_HEIGHT - 15 - 5, f"Forma: {seed}")

    canvas_pdf.setFont("Helvetica", 12)
    canvas_pdf.drawRightString(240, MAX_PAGE_HEIGHT - 15 - 5, f"https://ensayosaleatorios.streamlit.app/")
...
def add_pdf_page(canvas_pdf, layout_page, categories_list, seed):
    ...
    space_left = MAX_PAGE_HEIGHT - TOP_BOTTOM_MARGIN - ITEM_MARGIN
    for img_route in layout_page:
        ...
        canvas_pdf.setLineWidth(1)
        space_left -= new_height + ITEM_MARGIN
...
if __name__ == '__main__':
    ...
    with st.container(border=True):
        ...
        if st.button("Obtener pdf"):
            ...
            random_elements_per_category, categories_list, seed = get_random_elements('./database', n_questions, seed_input)
            st.session_state["random_elements_per_category"] = random_elements_per_category
        ...
        if "pdf_buffer" in st.session_state:
            st.success(f"Se creó un ensayo con la forma: {st.session_state['seed']}. Ahora puedes descargarlo.")
            st.download_button(
